...
MySQL.ready(function()
    print("^5[SIMPLE-SCRIPTS] ^2[INFO]^7 Die ESX-Datenbank ist verbunden.")
end)
            ...
            if jobChangesWithinThreshold == 0 then
                MySQL.Async.execute(
                    ...
                    function(rowsChanged)
                        ...
                        SendDiscordMessageWithEmbed(embed) -- Sende Discord-Nachricht mit eingebettetem Format
                    end
                )
            else
...
AddEventHandler('esx:setJob', function(playerId, job, lastJob)
    ...
    if player then
        -- Überprüfung, ob sowohl der alte als auch der neue Job in der Liste der unterstützten Jobs enthalten sind
...
ESX.RegisterServerCallback('getFraksperreData', function(source, cb)
    ...
    -- Berechnung des Datums 3 Tage in der Zukunft
    ...
    MySQL.Async.fetchAll('SELECT Fraksperre_ID, Datum FROM fraksperre WHERE Spieler_ID = @playerId ORDER BY Datum DESC LIMIT 1', {
        ...
        if result[1] then
            cb(result[1].Fraksperre_ID, futureDate) -- Rückgabe der Fraksperre-ID und des zukünftigen Datums
        else
            cb(nil, futureDate) -- Rückgabe von nil für Fraksperre-ID und zukünftigem Datum, wenn kein Eintrag gefunden wurde
        end
    end)
end)

-- Hilfsfunktion zur Überprüfung, ob ein Job unterstützt wird
        ...
        if player.getGroup() == SimpleScripts.FraksperrenAdminGroup then 
            ...
            if targetId then
                ...
                MySQL.Async.fetchScalar(
                    ...
                    {
                        ['@characterId'] = characterId  -- Änderung hier, um die Charakter-ID zu verwenden
                    },
